...
dbPath = os.path.join('db', 'violations.db')
...
class ImageWindow(QMainWindow):
    ...
    def showImage(self, imagePath: str):
        self.image.setText("")
        self.image.setPixmap(QtGui.QPixmap(imagePath))
        self.image.setGeometry(QtCore.QRect(0, 0, 841, 511))
...
class TabsWidget(QWidget):
    ...
    def initStudentTab(self):
        ...
        self.thread.warningsSignal.connect(self.updateWarnings)
        self.thread.updatePlotSignal.connect(self.updatePlotData)
        ...
        self.violationsLabel.setMaximumHeight(25)
        ...
        self.safeDistanceLabel = QLabel(Safe_Distance)
        self.safeDistanceLabel.setMaximumHeight(25)
        self.safeDistanceLabel.setAlignment(Qt.AlignCenter)
        self.thresholdLabel = QLabel(Threshold)
        self.thresholdLabel.setMaximumHeight(25)
        self.thresholdLabel.setAlignment(Qt.AlignCenter)

        self.warningsLabel = QLabel('Warnings')
        self.warningsLabel.setStyleSheet("border: 3px solid yellow")
        self.warningsLabel.setMaximumHeight(25)
        self.warningsLabel.setAlignment(Qt.AlignCenter)
        ...
        self.tab1Vertical.addWidget(self.safeDistanceLabel)
        self.tab1Vertical.addWidget(self.thresholdLabel)
        self.tab1Vertical.addWidget(self.warningsLabel)
    ...
    def initDatabaseTab(self):
        ...
        self.imageDialog = ImageWindow(self)
        data = cur.execute("SELECT * FROM violations").fetchall()
        if len(data) == 0:
            data = None

        self.tableButtonsLayout = QHBoxLayout()
        self.viewImageBtn = QPushButton('View Image')
        self.deleteBtn = QPushButton('Delete Record')
        self.viewImageBtn.clicked.connect(self.viewImage)
        ...
        self.table.setMaximumWidth(1280)
        header = self.table.horizontalHeader()
        header.setSectionResizeMode(3, QHeaderView.ResizeToContents)

        self.tableButtonsLayout.addWidget(self.viewImageBtn)
        self.tableButtonsLayout.addWidget(self.deleteBtn)

        self.tab2.layout = QVBoxLayout()
        self.tab2.layout.addLayout(self.tableButtonsLayout)
        # self.tab2.layout.addWidget(self.viewImageBtn)
        # self.tab2.layout.addWidget(self.deleteBtn)
        self.tab2.layout.addWidget(self.table)
        self.tab2.layout.setAlignment(QtCore.Qt.AlignCenter)
        self.tab2.setLayout(self.tab2.layout)

    def showSelection(self, item):
        id = self.table.currentIndex().siblingAtColumn(0).data()
        print(id)
        self.selectedId = id
    ...
    def viewImage(self):
        if self.selectedId is not None:
            data = cur.execute(
                f'SELECT * FROM violations WHERE ID={self.selectedId};').fetchone()
            self.imageDialog.showImage(data[4])
            self.imageDialog.show()
        # print("{:%B %d %H:%M, %Y}".format(datetime.datetime.now()))

    def deleteRecord(self):
        cur.execute(f'DELETE FROM violations WHERE ID={self.selectedId};')
        conn.commit()

        data = cur.execute("SELECT * FROM violations").fetchall()
        modelUpdate = TableModel(data)
        self.table.setModel(modelUpdate)
        self.table.update()

    @pyqtSlot(int)
    def updatePlotData(self, time_elapsed):
        self.dialog.onDateChange()

    @pyqtSlot(int)
    def updateWarnings(self, warnings):
        # print('violations: ',violations)
        self.warningsLabel.setText(f'Warnings: {warnings}')
...
class VideoThread(QThread):
    ...
    warningsSignal = pyqtSignal(int)
    updatePlotSignal = pyqtSignal(int)
    ...
    def run(self):
        ...
        while True:
            ...
            if (time_elapsed / 60) >= 1 and self.isActive and self.violationsEvery10Mins > 0:
                createdAt = datetime.datetime.now()
                params = (self.violationsEvery10Mins,
                          self.warningsEvery10Mins, createdAt, self.prevDatePath)
                cur.execute(
                    "INSERT INTO violations VALUES (NULL, ?, ?, ?, ?)", params)
                conn.commit()
                self.violationsEvery10Mins = 0
                self.warningsEvery10Mins = 0
                self.prevDatePath = None
                ...
                self.updatePlotSignal.emit(time_elapsed)
            ...
            # date = "{:%B %d %H %M %Y}".format(datetime.datetime.now())
            # dateStr = "-".join(date.split(' '))
            # newDatePath = "images/" + dateStr + ".jpg"
            # cv2.imwrite(newDatePath, frame)
            # # if self.prevDatePath is not None:
            # #     try:
            # #         os.remove(self.prevDatePath)
            # #     except FileNotFoundError:
            # #         print('file not found')
            # self.prevDatePath = newDatePath
            # createdAt = datetime.datetime.now()
            # params = (self.violationsEvery10Mins,
            #           self.warningsEvery10Mins, createdAt, self.prevDatePath)
            # cur.execute(
            ...
            if self.isActive:
                self.violationsEvery10Mins = max(
                    self.violationsEvery10Mins, len(serious))
            ...
            if self.isActive:
                self.warningsSignal.emit(len(abnormal))
                self.warningsEvery10Mins = max(
                    self.warningsEvery10Mins, len(abnormal))
...
class TableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, data, columns=['Index', 'Violations', 'Warnings', 'Date', 'Image']):
